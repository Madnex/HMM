...
Now the EM-Algrithm will be used to fit different $M$-state Hidden Markov Models to each data set. We will consider only Markov Chains with two and three states. We can then plot the estimates hidden states and the posterior probabilities.
...
hmm_GSPC <- lapply(2:3,function(i){depmix(Returns ~ 1, 
                                          family = gaussian(), 
                                          nstates = i,data=data.frame(Close_GSPC$Returns))})
...
for(i in 1:2){
  plot(postprobs_GSPC[[i]]$state, type='s', 
       main=paste('Estimated regimes GSPC for ',i+1," variables"), ylab='Regime')
  matplot(postprobs_GSPC[[i]][, -1], type='l', 
          main=paste('Posterior Probabilities GSPC for ',i," variables"),ylab='Probability')
...
for(i in 1:2){
  plot(postprobs[[i]]$state, type='s', 
       main=paste('Estimated regimes IWM for ',i+1," variables"), ylab='Regime')
  matplot(postprobs[[i]][, -1], type='l',
          main=paste('Posterior Probabilities IWM for ',i," variables"),ylab='Probability')
  legend(x='bottomright', legend=c(1:(i+1)), fill=1:(i+1), bty='n')
}
```
...
# GSPC
# AIC
suppressWarnings(sapply(hmmfit_GSPC, AIC)) # To avoid warning messages
# BIC
...
plot(postprobs_GSPC[[2]]$state, type='s',
     main=paste('Estimated regimes GSPC for ',3," variables"), ylab='Regime')
plot(postprobs[[2]]$state, type='s', 
     main=paste('Estimated regimes IWM for a model with ',3," variables"), ylab='Regime')
```
...
plot(postprobs_GSPC[[1]]$state, type='s', 
     main=paste('Estimated regimes GSPC for ',2," variables"), ylab='Regime')
plot(postprobs[[1]]$state, type='s', 
     main=paste('Estimated regimes IWM for a model with ',2," variables"), ylab='Regime')
```
