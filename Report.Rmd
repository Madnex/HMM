...
* A further generalisation technique would be trying to use the exact same model on datasets from another countries and see it's performance, comparing its performance on data from one country with that on data from another country may provide clues to what parts of the models have a general value (they speak about the evolution of stock exchanges in general) and what parts are heavily influenced by the particularities of the data used for fitting.